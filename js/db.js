// js/db.js - إصدار معدل لتحميل sql.js بشكل صحيح
...
async function loadSqlJs() {
    // انتظار تحميل المكتبة من script في index.html
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            clearInterval(check);
            reject(new Error('لم يتم تحميل مكتبة SQL.js'));
        }, 10000);
    });
}
...
async function initDatabase() {
    ...
    try {
        ...
        // تحديد مسار ملف wasm بطريقة متوافقة مع Windows و Linux
        const locateFile = (file) => {
            // استخدام المسار النسبي من المجلد الجذر
            return `lib/${file}`;
        };
        ...
        console.error('خطأ في initDatabase:', err);
...
// تصدير الدوال للإستخدام المباشر
